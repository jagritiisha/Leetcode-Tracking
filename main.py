...
dataframe = pd.DataFrame(
    columns=["Enrolment Number", "Name", "Leetcode ID", "Rating"])
action_dataframe = pd.DataFrame(columns=[
                                "Enrolment Number", "Name", "Leetcode ID", "Rating", "Contests Missed", "Number of contest missed"])
...
with requests.Session() as session:
    ...
    for username in usernames:
        ...
        headers = {"Referer": "https://leetcode.com/" + username + "/",
                   "Content-Type": "application/json", "X-CSRFToken": csrf_token}

        # send the request
        response = session.post(
            LEETCODE_URL, json=request_body, headers=headers)
        ...
        if data["data"]["userContestRanking"] is not None:
            ...
            for contest in contest_history:
                if contest["contest"]["startTime"] > start_date:
...
action_dataframe.to_csv("action.csv", index=False)